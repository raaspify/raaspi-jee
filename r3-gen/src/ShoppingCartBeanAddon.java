...
public class ShoppingCartBeanAddon extends ShoppingCartBean implements java.io.Serializable {
    ...
    viewId =  FacesContext.getCurrentInstance().getViewRoot().getViewId();
     ...
     if (httpreq.getParameter("data") != null ){
      try{
       ...
       log.info("#{messages['square_ios_order/url']} "+ cartOrder+" "+owner2_url);
     ...
     if (httpreq.getParameter("com.squareup.pos.REQUEST_METADATA") != null ){//android also gets owner2 via REQUEST_METADATA(state)
      try{
       ...
       log.info("#{messages['square_android_order/url']} "+ cartOrder+" "+owner2_url);
       ...
       log.severe(e.getMessage());
       FacesContext.getCurrentInstance().addMessage(null,new FacesMessage(
        FacesMessage.SEVERITY_ERROR,e.getMessage()+" "+bundle.getString("in")+" "+bundle.getString("squarel")+" "+bundle.getString("response")+" "+bundle.getString("handling")+", "+bundle.getString("try")+" "+bundle.getString("again"),""));
       bcontinue=false;
       try{
         extCtxt.redirect(owner2_url);
       }catch(Exception ex){
 ...
 public void init(String owner2Code){
		...
		// done as Init Servlet.
           ...
           String clCurrSymb=yxxxch522xhhxxhxxxxxclient.get(0).getJ9xxuxxxbvxxxxxxxxxxcurrsymb();
           if (clCurrSymb != null && !clCurrSymb.isEmpty()){
            currSymb=clCurrSymb;
           }

           clCurrCode=yxxxch522xhhxxhxxxxxclient.get(0).getY8xxuxxxbvxxxxxxxxxxdefaultc();
           if (clCurrCode != null && !clCurrCode.isEmpty()){
            defaultc=clCurrCode;//item/customer
           }
           clCurrSymb=yxxxch522xhhxxhxxxxxclient.get(0).getY2xxuxxxbvxxxxxxxxxxkeyorder();
           if (clCurrSymb != null && !clCurrSymb.isEmpty()){
            keyorder=clCurrSymb;
           }
 ...
 public void confirmPDPayment(){
  ...
  yxxxch522xhhxxhxxxxxclient = yxxxch522xhhxxhxxxxxclientList.getResultList();
  if(yxxxch522xhhxxhxxxxxclient.get(0).getD9xxuxxxbvxxxxxxxxxxenterprisecompsize()==null ||
     yxxxch522xhhxxhxxxxxclient.get(0).getD9xxuxxxbvxxxxxxxxxxenterprisecompsize().isEmpty() || yxxxch522xhhxxhxxxxxclient.get(0).getD9xxuxxxbvxxxxxxxxxxenterprisecompsize().equals("S")){
 ...
 // regular confirmpayment should also allow for adjustment if item out of stock/backorder
      ...
      if (currentCartOrder.size() >0){
       ...
       trexuq244xwwqqhxxxxxcuorders.setC2xxcxxxbv09xxxxxxxxterms("PICKUP");
 ...
 // at end callss itemHome.updateInvoicesToPay
 // Finally sends emails as confirmation to user and client
 // nov26 2019 if pos then skip invoices just payment as if by chq
      String type="cart";
               ...
               try{
                 ...
                 if (fpxxfq1a5xwwqqhxxxxxitemList.getNetCartedPayTotal("cart").compareTo( new BigDecimal("0.0"))==0){
                        ...
                        vEmailAddress=currentCartOrder.get(0).getJxxxuq201xwwqqhxxxxxcustomer().getQ4xxhxxxbv24xxxxxxxximailaddr();
                        if(vEmailAddress == null || vEmailAddress.isEmpty()){
                         vEmailAddress=payerEmail;//nov 13 2019 created in makePayPalPayment step 2.b but session will hold it
...
// (3a) payment execution request via payment module execute method to move money from buyer to merchant
		     ...
		     //req.getRequestDispatcher("response.jsp").forward(req, resp);
                     ...
                     transactionId="jay";
                     //following calls cartExtended_persist to create invoice
 ...
 public void makePayPalPayment(String payerid,String guid) throws Exception
 {
 		 ...
 		 lName=payment.getPayer().getPayerInfo().getLastName();
                 payerEmail=payment.getPayer().getPayerInfo().getLastName();//nov 13 2019
 ...
 public void saleNAproveRequest() throws Exception
  ...
  try{
   ...
   //following needs to be called via action method ie simulate login, confirm payment seq
   //confirmPayment();
   return;
  } catch (Exception e) {
    		     log.severe(e.getMessage());
 ...
 {         // (1) action by checkout saleRequest button Paypal Purchase , variable values are session scope ie value held
		...
		try {
			 ...
			 */
                ...
                //jul21 2020 replaced pay with cart but check how pos/cod/cop can come here?
                List<Trexuq244xwwqqhxxxxxcuorders> currentCartOrder = fpxxfq1a5xwwqqhxxxxxitemList.getCurrentCartOrder(type);
			...
			amount.setCurrency(defaultc);//feb16 2020
			// Total must be equal to sum of shipping(delivery charge) , tax (tax1) and subtotal (baseamount).
                        ...
                        // online payment not meant for COD
			...
			// to a List
                ...
                for (Xoxxuq2b4xwwqqhxxxxxcuodetails itemOrderDetail : fpxxfq1a5xwwqqhxxxxxitemList.getCartedList("cart")) {
                 ...
                 if(!itemOrderDetail.getFpxxfq1a5xwwqqhxxxxxitem().getC6xxusxrbv16xxxxxxxxtype().equals("SI-C")){
                  ...
                  item.setCurrency(defaultc);//feb16 2020
			...
			guid = UUID.randomUUID().toString().replaceAll("-", "");
                        ...
                        if(yxxxch522xhhxxhxxxxxclient.get(0).getZaxxuxxxssxxxxxxxxxxpop3pass() !=null
                          && !yxxxch522xhhxxhxxxxxclient.get(0).getZaxxuxxxssxxxxxxxxxxpop3pass().isEmpty()){
                         revisedUrl=yxxxch522xhhxxhxxxxxclient.get(0).getZaxxuxxxssxxxxxxxxxxpop3pass()+".ngrok.io";
                        }
      ...
      // action by customer shipping edit after shipping to capture payment which has been authorized
		...
		try {
			...
			// A capture transaction
                        ...
                        if(accessToken == null || accessToken.isEmpty()){//dec01 2020 added null check
                         init("demo");
                        }
			APIContext apiContext = new APIContext(accessToken);
...
// (2) express payment request via  checkout.xhtml link button value approvalUrl / login to paypal
 ...
 // which in turn calls this final method
 // if netPay is zero then assume its cod/pos and paypal payment authorization is bypassed
 // jan02 2019 COD payment now part of backoffice inperson/POS payment not part customer pay flow
