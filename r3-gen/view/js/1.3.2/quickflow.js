	...
	window.jsPlumbDemo = {
		init : function() {
			...
			init = function(connection) {
                        ...
                        switch (sourceTarget) {
                               case "window4window3":
                                connection.labelText = "  Order ";
                                break;
                               case "window1window3":
                                connection.labelText = "......Receive ";
                                break;
                               case "window3window2":
                                connection.labelText = " Assign  ";
                                break;
                               case "window3window4":
                                connection.labelText = "  Mail/Ship ";
                                break;
                               case "window3window1":
                                connection.labelText = "  Order ";
                                break;
                               case "window2window3":
                                connection.labelText = "  Deliver.....";
                                break;
			...
			jsPlumb.bind("click", function(conn) {
						if (confirm("Go to Menu Option "+conn.labelText + "?")){
                                         if (conn.labelText=="  Order " &&  conn.sourceId=="window4"){
                                          window.location.href=("cuordersEdit.wflow");
                                         }
                                         if (conn.labelText=="  Mail/Ship " &&  conn.targetId=="window4"){
                                          window.location.href=("shipmentEdit.wflow");
                                         }
                                         if (conn.labelText=="  Order....." &&  conn.sourceId=="window2"){
                                          window.location.href=("cuordersEdit.wflow");
                                         }
                                         if (conn.labelText==" Assign  " &&  conn.targetId=="window2"){
                                          window.location.href=("emplordersEdit.wflow");
                                         }
                                         if (conn.labelText=="  Order " &&  conn.targetId=="window1"){
                                          window.location.href=("vendordersEdit.wflow");
                                         }
                                         if (conn.labelText=="......Receive " &&  conn.sourceId=="window1"){
                                          window.location.href=("vshipmenEdit.wflow");
                                         }
                               }
